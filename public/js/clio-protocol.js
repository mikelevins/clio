...
function handleCreateButton(envelope) {
    ...
    let parentId = envelope['into'] || 'main-container';
    let parent = document.getElementById(parentId);
    if (!parent) {
        console.warn(
            `Clio: handleCreateButton could not find parent element ` +
            `with id "${parentId}". Button (id=${elementId}) was not ` +
            `appended. Envelope:`,
            envelope
        );
        return;
...
function handleCreateInput(envelope) {
    ...
    if (!parent) {
        console.warn(
            `Clio: handleCreateInput could not find parent element ` +
            `with id "${parentId}". Input (id=${elementId}) was not ` +
            `appended. Envelope:`,
            envelope
        );
        return;
